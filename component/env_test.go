...
func TestLoadEnvFromSystemEnvVar(t *testing.T) {
	...
	_, exist := os.LookupEnv("Consumer_Net_ListenPort")
	assert.False(t, exist)
	_, exist = os.LookupEnv("Consumer_Activity_Batch")
	assert.False(t, exist)
	setupEnvVars(t)
	defer teardownEnvVars(t)
	port, exist := os.LookupEnv("Consumer_Net_ListenPort")
	assert.True(t, exist)
	assert.Equal(t, "8081", port)
	batch, exist := os.LookupEnv("Consumer_Activity_Batch")
	assert.True(t, exist)
